...
namespace IngameScript
{
    ...
    public abstract class TurretDriver
    {
        public string Name;
        ...
        public virtual iniWrap Setup(ref CombatManager m)
        {
            ...
            m.Terminal.GetBlocksOfType<IMyMotorAdvancedStator>(null, (b) =>
            {
                if (b.CubeGrid.EntityId != id) return false;
                if (b.CustomName.Contains(el))
                    Elevation = b as IMyMotorAdvancedStator;
                return true;
            });
            if (Elevation != null)
    ...
    public class GunTurret : TurretDriver
    {
        ...
        public override iniWrap Setup(ref CombatManager m)
        {
            ...
            p.Dispose();
            var list = new List<IMyFunctionalBlock>();
            m.Terminal.GetBlockGroupWithName(Name + " Weapons").GetBlocksOfType(list); // e.g. PDL-LT Guns
        ...
        public override void SelectTarget(ref Dictionary<long, Target> targets)
        {
            ...
            if (spray && selOffset)
            {
                randOffset = new Vector3D(((Host.rand.NextDouble() * 2) - 1), ((Host.rand.NextDouble() * 2) - 1), ((Host.rand.NextDouble() * 2) - 1)) * scat;
                selOffset = false;
            }
            ...
            foreach (var target in targets.Values)
            {
                ...
                var tgtP = target.PositionUpdate(Host.Time);
