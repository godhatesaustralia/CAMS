...
namespace IngameScript
{
    partial class Program : MyGridProgram
    {
        ...
        enum PostSetupAction { None = 0, Fire = 1, Handshake = 2 };
        ...
        const string
            ...
            IgcTagSplash = "IGC_MSL_SPLASH_MSG",
            IgcTagInit = "IGC_MSL_RDY_MSG",
            IgcTagStatus = "IGC_MSL_STAT_MSG",
        ...
        ConfigString
            ...
            _mainframeName = new ConfigString(IniSectionMain, "CAMS mainframe block name", "Computer CAMS"),
            _detachThrustTag = new ConfigString(IniSectionMain, "Detach thruster name tag", "Detach");
            ...
            _guidanceAlgorithms = new Dictionary<GuidanceAlgoType, MissileGuidanceBase>()
{
    ...
    { GuidanceAlgoType.ZeroEffortMiss, new ZeroEffortMissGuidance(UpdatesPerSecond, _navConstant) },
        ...
        void IgcMessageHandling(bool shouldFire)
        {
            if (!shouldFire)
            {
                ...
                while (_unicastListener.HasPendingMessage)
                {
                    ...
                    // else if (message.Tag == IgcTagStatus)
                    // {
                    //     if (data is long && _master == -1)
                    //         _master = (long)data;
                    // }
        ...
        IEnumerator<SetupStatus> SetupStateMachine(bool reload = false)
        {
            ...
            LoadIniConfig();
            _missileGroupNameTag = $"{_missileTag}-{_missilePos} WHAM";
            ...
            else if (!reload)
            {
                ...
                if (_master == -1)
                {
                    ...
                    _setupBuilder.Append($">> ERR: Could not find '{_mainframeName}'! This missile MUST be connected to a CAMS mainframe to fire!\n");
                }
            }
            #endregion
        ...
        public void RunSetupStateMachine()
        {
            ...
            if (_setupStateMachine != null)
            {
                ...
                else // Done with block fetching
                {
                    ...
                    if ((_postSetupAction & PostSetupAction.Handshake) != 0)
                    {
        ...
        {
            _missileStage = 2;
            _launchHeadingVector = _offsetReference != null ? _missileReference.WorldMatrix.Translation - _offsetReference.WorldMatrix.Translation : _missileReference.WorldMatrix.Forward;
        ...
        void SteerToLaunchVector()
        {
            double yaw, pitch, roll;
            var missileMatrix = _missileReference.WorldMatrix;
            GetRotationAnglesSimultaneous(ref _launchHeadingVector, missileMatrix.Up, ref missileMatrix, out pitch, out yaw, out roll);
        ...
        void Control(MatrixD missileMatrix, Vector3D accelCmd, Vector3D gravityVec, Vector3D velocityVec, double mass)
        {
            ...
            double yaw, pitch, roll;
            GetRotationAnglesSimultaneous(ref accelCmd, -gravityVec, ref missileMatrix, out pitch, out yaw, out roll);
        ...
        public static void GetRotationAnglesSimultaneous(ref Vector3D desiredForwardVector, Vector3D desiredUpVector, ref MatrixD worldMatrix, out double pitch, out double yaw, out double roll)
        {
        ...
        ass rotation axes. 
        */
        void ApplyGyroOverride(double pitchSpeed, double yawSpeed, double rollSpeed, List<IMyGyro> gyroList, ref MatrixD worldMatrix)
        {
